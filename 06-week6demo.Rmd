...
# create skipgrams
tidy_skipgrams <- emails %>%
  unnest_tokens(ngram, value, token = "ngrams", n = 6) %>%
...
# SVD
pmi_svd <- svds(pmi_matrix, 50)

word_vectors <- pmi_svd$u
rownames(word_vectors) <- rownames(pmi_matrix)
```
At this stage, I apply Singular Value Decomposition (SVD) to the PMI matrix in order to generate word embeddings. The PMI matrix captures how strongly words co-occur relative to chance, and SVD reduces this high-dimensional matrix into a lower-dimensional space.
The resulting object, `word_vectors`, represents each word as a numeric vector. Words that appear in similar contexts will have similar vector representations. This allows us to measure semantic similarity between words using vector operations, such as the dot product.
In this model, each word is represented as a 50-dimensional vector. This dimensionality is a design choice that balances capturing semantic variation while keeping the model computationally efficient.

```{r}
dim(word_vectors)
```
The resulting word embedding matrix contains 844 unique words, each represented as a 50-dimensional vector. This means that every word in the corpus is mapped onto a point in a 50-dimensional semantic space. Words that appear in similar contexts are expected to have similar vector representations, allowing us to measure semantic similarity between words.
...
```{r}
nearest_words(word_vectors, "government")
nearest_words(word_vectors, "university") %>% head(10)
nearest_words(word_vectors, "staff") %>% head(10)
nearest_words(word_vectors, "students") %>% head(10)
nearest_words(word_vectors, "financial") %>% head(10)
```
To explore the semantic structure captured by the word embeddings, I examined the nearest neighbours of several key terms in the corpus, including "university", "staff", and "government".
The results for "university" show a relatively coherent semantic cluster, with closely related words such as "college", "union", "finances", and "executive". These terms reflect the institutional and administrative context of the emails, suggesting that the model successfully captures meaningful associations between words that frequently co-occur in similar contexts.
Similarly, the term "staff" is associated with words such as "reduce", "costs", "operating", and "workstream", which reflect discussions of organisational change and financial pressures within the university. This indicates that the embedding is able to capture policy-related and managerial themes present in the text.
In contrast, the results for "government" are less coherent, with many associated terms being generic or weakly related. This suggests that the model struggles to capture clear semantic relationships for some words.
Overall, these differences likely reflect the limited size of the corpus, which consists of only a small number of emails. With fewer co-occurrence patterns available, the model has less information to learn stable word associations, leading to weaker or noisier embeddings for some terms.
...
colnames(umap_df) <- c("UMAP1", "UMAP2", "word")
```
```{r}
ggplot(umap_df, aes(x = UMAP1, y = UMAP2)) +
  geom_point(alpha = 0.5) +
  theme_minimal() +
  labs(title = "Word Embedding Visualization (UMAP)")
```
The UMAP visualization shows the structure of the word embeddings in two-dimensional space. A dense cluster of points can be observed in the centre of the plot, indicating that many words share similar contextual usage and are therefore grouped closely together in the embedding space. These are likely high-frequency words related to the core themes of the emails, such as organisational processes, staff, and institutional management.
In contrast, many words are distributed along curved patterns at the edges of the plot. These words are likely less frequent or appear in more specific contexts, resulting in weaker or more distinct semantic relationships. As a result, they are positioned further away from the central cluster.
Additionally, some smaller isolated clusters can be observed, suggesting the presence of more specialised or context-specific vocabulary. Overall, the visualization reflects how the embedding model captures both central themes and peripheral terms within the corpus.
```{r}
subset_words <- umap_df %>%
  filter(UMAP1 > -1 & UMAP1 < 2 & UMAP2 > 3 & UMAP2 < 6)

ggplot(subset_words, aes(x = UMAP1, y = UMAP2)) +
  geom_point() +
  geom_text(aes(label = word), size = 3, vjust = -1) +
  theme_minimal() +
  labs(title = "Zoomed-in Cluster of Core Words")
```
The zoomed-in UMAP visualization reveals that the central embedding space is structured into several meaningful semantic sub-clusters rather than forming a single homogeneous group.
One cluster, located near the centre of the plot, includes words such as "workstream", "change", and "projects", which relate to organisational processes and institutional restructuring. This reflects the focus of the emails on internal reforms and operational adjustments.
A second cluster, positioned towards the right-hand side, contains words such as "jobs", "funding", and "finances", indicating a distinct semantic area related to financial pressures and cost reduction. This aligns with the recurring theme of financial sustainability present throughout the corpus.
A third cluster, located in the upper region of the plot, includes terms such as "sustainability", "impact", and "university", which reflect broader institutional goals and long-term strategic priorities.
Finally, a more diffuse cluster on the left-hand side contains more generic terms such as "make", "people", and "working", which are less semantically specific and likely reflect general language use.
Overall, this visualization demonstrates that the word embedding model is able to capture meaningful semantic structure within the text, separating different thematic dimensions such as management processes, financial concerns, and institutional objectives into distinct regions of the embedding space. This also highlights the usefulness of dimensionality reduction techniques such as UMAP in making high-dimensional semantic relationships interpretable.
